...
float k_d = 5;
float k_p = 4;
float time_coef = 0.05;
float correct_coef = 0.5;
...
void getAngle(const nav_msgs::Odometry::ConstPtr &msg) {
    ...
    angleReal = atan2(msg->pose.pose.orientation.z, msg->pose.pose.orientation.w) * 2;
    /*
...
{
    float Shift = -2;
    return (cos(t * time_coef) + cos(2*t * time_coef) + Shift) * correct_coef;
}

float Xdt(float t)
{
    return (-time_coef*sin(t * time_coef) - 2*time_coef*sin(2*t * time_coef)) * correct_coef;
}

float Xdtdt(float t)
{
    return (-time_coef*time_coef*cos(t * time_coef) - 4*time_coef*time_coef*cos(2*t * time_coef)) * correct_coef;
}

float Y(float t)
{
    return 2*sin(t * time_coef) * correct_coef;
}

float Ydt(float t)
{
    return 2*time_coef*cos(t * time_coef) * correct_coef;
}

float Ydtdt(float t)
{
    return -2*time_coef*time_coef*sin(t * time_coef) * correct_coef;
}
...
int main(int argc, char *argv[])
{
    ...
    messageMotion.angular.z = PI / 2 / 3;
	...
	while (timeReal < 2.0 * PI / time_coef)
	{
        ...
        ROS_INFO("V=%f, W=%f", velocity, omega);
	}
